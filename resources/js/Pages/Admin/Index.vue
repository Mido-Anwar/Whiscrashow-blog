...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="flex flex-col w-full px-2 mt-2">
            <Link class="bg-blue-500
                                 hover:bg-blue-400
                                 text-white font-bold p-2 border-b-4
                                  border-blue-700
                                   hover:border-blue-500 rounded w-1/4" :href="route('user.create')">
            Create New User
            </Link>
            ...
            <table class="table-auto mt-4 border border-black">
                ...
                <tbody>
                    <tr v-for="user in users" :key="user.id">
                        <td class="px-4 py-2 border border-black">{{ user.id }}</td>
                        <td class="px-4 py-2 border border-black">{{ user.name }}</td>
                        <td class="px-4 py-2 border border-black">{{ user.email }}</td>
                        <td class="px-4 py-2 border border-black">{{ user.role }}</td>
            ...
            <Link class="bg-green-500 hover:bg-green-400
                                     text-white font-bold p-2 border-b-4
                                      border-green-700
                                     hover:border-green-500 rounded w-1/4" :href="route('role.create')">
            Create New Role
            </Link>
            ...
            <table class="table-auto mt-4 border border-black">
                ...
                <tbody>
                    <tr v-for="role in roles" :key="role.id">
                        <td class="px-4 py-2 border border-black">{{ role.id }}</td>
                        <td class="px-4 py-2 border border-black">{{ role.name }}</td>
                        <td class="px-4 py-2 border border-black">{{ role.guard }}</td>
                        <td class="px-4 py-2 border border-black">
            ...
            <table class="table-auto mt-4 border border-black">
                ...
                <tbody>
                    <tr v-for="permission in permissions" :key="permission.id">
                        <td class="px-4 py-2 border border-black">{{ permission.id }}</td>
                        <td class="px-4 py-2 border border-black">{{ permission.name }}</td>
                        <td class="px-4 py-2 border border-black">{{ permission.guard }}</td>
                        <td class="px-4 py-2 border border-black">
